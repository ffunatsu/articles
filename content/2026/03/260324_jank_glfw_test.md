...
また、`not`と`cpp/!` が違う（特に`cpp`の値を渡したときに`not`だと反転されない）のにはかなり戸惑った。たぶんだけど `not` な `cpp-value` というのは、何らかのC++の値があれば true 判定でそれが not で false になるっぽくて、極端なことをいえば `cpp/false` でも `(cpp/int 0)`  でも not すると true になって頭がおかしくなりそうだった。なのでC++で not したいときは `cpp/!` を使うことを学んだ。
