...
https://github.com/ffunatsu/jank-stb_image-test/tree/dev
...
stb_imageのほうはClojurians Slackで報告があがっているように妙なエラーが出ていて、工夫をしないと動かなかったけれど、一応無事動いた。（devブランチの方がストレートな実装。mainにマージしたいけれどSlackのコメントとの流れを鑑みて一旦保留。）
...
ちなみにjankに移植して一番大変だったのは、jankの関数呼び出しが型にかなり厳密だということ。例えばC++なら`NULL`で呼び出せるところが`nullptr`じゃないとポインタ型の呼び出しだと判定されずに型エラーで関数が呼び出せなかったりなどは日常茶飯事。

また、`not`と`cpp/!` が違う（特に`cpp`の値を渡したときに`not`だと反転されない）のにはかなり戸惑った。たぶんだけど `not` な `cpp-value` というのは、何らかのC++の値があれば true 判定でそれが not で false になるっぽくて、極端なことをいえば `cpp/false` でも `(cpp/int 0)`  でも not すると true になって頭がおかしくなりそうだった。なのでC++で not したいときは `cpp/!` を使うことを学んだ。（開発中の言語なので、今後は定義が変わっていくかもしれない。）
