...
ちなみにTagged UnionはUnionに列挙したなかで最長のメモリサイズを事前に確保することが多いので、場合によってはメモリが無駄な実装になることもある。（上記の例のようにポインタ = `uintptr_t` という同じサイズのものであれば問題にはならないけれど、ポインタなら別に実体を置く場所が必要になる。）
## まとめ
