...
ただ欠点としては、万能型というのがありえなくなるので、型だらけになる[^1]。でも（言語標準の）Tagged UnionやVariantは動的型よりも実行効率が良いことが多いので、一長一短といえる。

ちなみにTagged UnionはUnionに列挙したなかで最長のメモリサイズを事前に確保することが多いので、場合によってはメモリが無駄になることもある。（上記の例のようにポインタ = `uintptr_t` という同じサイズのものであれば問題にはならないけれど、ポインタなら別に実体を置く場所が必要になる。）
## まとめ
...
オーバーヘッドと感じるかどうかは正直言語デザインによるので、使っている言語に引っ張られる部分はかなりあると思うし、ClojureやRuby、Lua等のように、動的型を前提として便利な記法やライブラリが存在しているかも大きな部分だと思う。

[^1]: JSONオブジェクトのライブラリも結局はTagged Unionで実装されているので、万能というわけではないのは同じ。