...
エラーの原因は、`run-main` の箇所だった。元は、`jank run-main xxxx yyyy ....` と呼ばれているのだけれど、最新のjankなのか、MSYS2版のjankなのかはわからないが、run-main xxxx はすべてのOption引数の後（かつ `--` の前）になければ正しく動作しない。
...
for idx in "${!pre_ddash[@]}"; do
    if [[ "${pre_ddash[$idx]}" == "run-main" ]]; then
        runmain_idx=$idx
        break
    fi
done
if [[ $runmain_idx -ge 0 ]]; then
    runmain="${pre_ddash[$runmain_idx]}"
    # 配列から物理的に削除
    tmp=()
    for idx in "${!pre_ddash[@]}"; do
        if [[ $idx -ne $runmain_idx ]]; then
            tmp+=("${pre_ddash[$idx]}")
        fi
    done
    pre_ddash=("${tmp[@]}")
