...
〇〇_jll はArtifactsをまとめているだけのリポジトリになっていて、〇〇_jll_packager.jl の方で実際の生成物のアップロードを行なっている。この2種はたぶん分離しなくてよい部分もあると思うのだけれど、後者のgitに生成物を既に直接アップロードしてあって、それを deploy.jl を使って一度 gist に tar.gz としてアップロードしている（実際にはこちらを利用する）ので、個人的にgitが重くなるのを避けたくて分離している。

〇〇_jll_packager.jl では本当に単にgistにアップロードしているだけで、アップロード後、一時的にArtifacts.tomlに参考情報が書き出される。次のアップロードでは上書きされてしまう仕組みになっているので、READMEにメモを書いている。
...
ちなみにArtifacts自体へのアクセスは、どうやらそのArtifactsを含むリポジトリのみがそのパスを知ることができるようなので、〇〇_jll のsrcにあるJuliaのコードからその辺のブリッジコードを書いている。

ブリッジコードとはいえ単に生成物へのパスを取得して返しているだけなので、その実際の読み込みなどは利用側でやっても問題ない様子。実際に今回の例では、最終的な利用側でCの読み込みコードは書いているけれど、これはどちらでやっても良いんじゃないかなと思う。
## まとめ
