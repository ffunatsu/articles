...
これと似たことができている言語はあとはRustくらいだし、しかも有名なパッケージかつ純粋なRustのみで、dll等を含むsysクレートは自前のビルド等に失敗しがちなので、そう考えるとどの言語にもできていない画期的な試みをLinuxパッケージマネージャ並に管理してマルチプラットフォーム展開しているのはすごいことだと思う。
