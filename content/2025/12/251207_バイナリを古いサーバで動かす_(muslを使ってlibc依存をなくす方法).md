...
という流れになる。以下、一つ一つ説明する。Odinを例にとっているが、Odinでなくても良い。（普通のC言語でもなんでも、`.o` が出力できるものなら可。）
...
実際、Odinの場合はどうなるかを説明する。ちなみに自分の場合は、以下のシンプルなHello Worldを使ったのだけど、それでも `.o` (オブジェクトファイル) は大量にできたので、フォルダを分けて出力させている。
